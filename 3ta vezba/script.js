class Calendar {
    ...
    loadEvents() {
        ...
        const events = savedEvents ? JSON.parse(savedEvents).map(event => ({
            ...event,
    ...
    renderEvent(event, isWeekView = false) {
        ...
        return `
            <div class="${eventClass}" onclick="event.stopPropagation(); calendar.openModal(new Date(${event.date.getFullYear()}, ${event.date.getMonth()}, ${event.date.getDate()}), ${JSON.stringify(event).replace(/"/g, '&quot;')})">
        ...
        let dayHTML = `
            <div class="day-view-timeline">
                ${Array.from({ length: 24 }, (_, hour) => {
                    ...
                    const isCurrentHour = selectedDate.toDateString() === new Date().toDateString() && new Date().getHours() === hour;
    
                    return `
                        <div class="hour-block ${isCurrentHour ? 'current-hour' : ''}" onclick="calendar.openModal(new Date(${selectedDate.getFullYear()}, ${selectedDate.getMonth()}, ${selectedDate.getDate()}))">
                            <div class="hour-label">${hourString}:00</div>
                            <div class="hour-event">
                                ${event ? `
                                    <div class="day-event" onclick="event.stopPropagation(); calendar.openModal(new Date(${event.date.getFullYear()}, ${event.date.getMonth()}, ${event.date.getDate()}), ${JSON.stringify(event).replace(/"/g, '&quot;')})">
                                        <span class="event-time">${this.formatEventTime(event.time)}</span>
                                        <span class="event-title">${event.title}</span>
                                    </div>
                                ` : ''}
        ...
        let weekHTML = `
            <div class="week-view">
                ${weekDates.map(date => {
                    ...
                    return `
                        <div class="week-day ${isToday ? 'current-day' : ''}" onclick="calendar.openModal(new Date(${date.getFullYear()}, ${date.getMonth()}, ${date.getDate()}))">
                            <div class="week-date">
                                <div class="week-day-name">${date.toLocaleDateString('default', { weekday: 'short' })}</div>
                                <div class="week-day-number">${date.getDate()}</div>
                            </div>
                            <div class="week-events">
                                ${events.map(event => this.renderEvent(event, true)).join('') || '<div class="no-events">No events</div>'}
                            </div>
                        </div>
                    `;
                }).join('')}
    ...
    renderMonthView() {
        const { startingDayIndex, daysInMonth, daysInPreviousMonth, year, month } = this.getMonthData();
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            daysHTML += `
                <div class="calendar-day current-month ${isToday ? 'current-day' : ''}" onclick="calendar.openModal(new Date(${year}, ${month}, ${day}))">
